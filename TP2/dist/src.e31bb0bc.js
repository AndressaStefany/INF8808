...
function cleanNames(data) {
  ...
  data.map(function (dataCSV) {
    var capLetter = dataCSV.Player.charAt(0).toUpperCase();
    var lwrLetters = dataCSV.Player.slice(1).toLowerCase();
...
function getTopPlayers(data) {
  ...
  var top5names = [];
  var top5namesOnly = [];
  var nameResultMap = new Map();
  data.forEach(function (dataCSV) {
    if (nameResultMap.has(dataCSV.Player)) nameResultMap.set(dataCSV.Player, nameResultMap.get(dataCSV.Player) + 1);else nameResultMap.set(dataCSV.Player, 1);
  });
  var nameResultMapSorted = new Map(_toConsumableArray(nameResultMap.entries()).sort(function (a, b) {
    return b[1] - a[1];
  }));
  var array = Array.from(nameResultMapSorted, function (_ref) {
    ...
    data.forEach(function (dataCSV) {
      if (dataCSV.Act == act) {
        if (!players.includes(dataCSV.Player)) {
          ...
          playerCount.push({
            Player: dataCSV.Player,
            Count: 1
          });
        } else {
          playerCount.forEach(function (count) {
...
function replaceOthers(data, top) {
  ...
  data.forEach(function (act) {
    var newPlayers = [];
    var countOther = 0;
    act.Players.forEach(function (player) {
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "8556" + '/');
